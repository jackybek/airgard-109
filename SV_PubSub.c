...
#define DATASETWRITERID			2234	// used in addDataSetReader() and addDataSetWriter()
 ...
 #define CA_FILE_PATH_MQTT                      "/etc/ssl/certs/mosq-ca.crt"		// symbolic link => /home/pi/ssl-certs-mosq/mosq-ca.crt
 #define CA_PATH_MQTT				"/etc/ssl/certs"
 #define CLIENT_CERT_PATH_MQTT			"/etc/ssl/certs/mosq-serv-22.crt"	// symbolic link => /home/pi/ssl-certs-mosq/mosq-serv-22.crt
 #define CLIENT_KEY_PATH_MQTT			"/etc/ssl/certs/mosq-serv-22.key"	// symbolic link => /home/pi/ssl-certs-mosq/mosq-serv-22.key
 //#define CLIENT_CERT_PATH_MQTT			"/etc/ssl/certs/brokerserver-11.crt"	// symbolic link => /usr/local/ssl/certs/brokerserver-11.crt
 //#define CLIENT_KEY_PATH_MQTT			"/etc/ssl/certs/brokerserver-key11.key"	// symbolic link => /usr/local/ssl/certs/brokerserver-key11.key
 // tested successfully on command line
 // sudo mosquitto_pub -d -h Mqtt-11 -p 8883 -t AirgardTopicCloudToEdge
 // --capath /etc/ssl/certs/ --cafile mosq-ca.crt --cert /etc/ssl/certs/brokerserver-22.crt --key /etc/mosquitto/certs/brokerserver-key22.key -m "Secure Message 2" --tls-version tlsv1.2 --insecure

 //#define CA_FILE_PATH_OPTION_NAME_MQTT          "mqttCaFilePath"
 //#define USE_TLS_OPTION_NAME_MQTT               "mqttUseTLS"
                 //UA_ByteString certificate = loadFile("/etc/ssl/certs/cert109.pem");   // => symbolic link
                //UA_ByteString certificate = loadFile("/usr/local/ssl/certs/cert109.pem");  // actual location
 ...
 */
	...
	// initiatise the value of PubSubconnectionIdentifier (UA_NodeId) so that we can check later in addSubscription()
    	printf("Before calling UA_Server_addPubSubConnection() : MQTT_Enable == UA_TRUE && AMQP_Enable == UA_FALSE\n");
	...
	*/
	...
	//sleep(5);
        ...
        // initiatise the value of PubSubconnectionIdentifier (UA_NodeId) so that we can check later in addSubscription()
        printf("Before calling UA_Server_addPubSubConnection() : MQTT_Enable == UA_FALSE && AMQP_Enable == UA_FALSE\n");
        UA_StatusCode retval = UA_Server_addPubSubConnection(uaServer, &connectionConfig, &PubSubconnectionIdentifier);
        if (retval == UA_STATUSCODE_GOOD)
        {
                UA_String output;
...
addWriterGroup(UA_Server *uaServer)
{
	...
	printf("addWriterGroup : MQTT_TLS_Enable = %d \n", MQTT_TLS_Enable);
